...
public class SupplierTable {
    public static Dataset<Supplier> GenerateTable(SparkSession session) {
        //字符串编码的模式
        //String schema = "C_CUSTKEY C_NAME C_ADDRESS C_NATIONKEY C_PHONE C_ACCTBAL C_MKTSEGMENT C_COMMENT";
        String schema = "s_suppkey s_name s_address s_nationkey s_phone s_acctbal s_comment";
        Encoder<Supplier> encoder = Encoders.bean(Supplier.class);
...
//                as(encoder);
        ...
        Dataset<Supplier> ds = session.
                read().
                schema(structType).
                option("header", "false").
                csv("D:\\SparkResource\\tpch_dataset\\supplier.csv").  //E:\source\1\customer.csv
                as(encoder);

        ds.createOrReplaceTempView("supplier");
        return ds;
    }
}
